<template>
    <nav class="h-24 bg-white border-b-2 border-select">
        <div class="w-full h-full px-[24px] flex items-center flex-row-reverse laptop:flex-row justify-between">
            <div  class="">
                <ul class="flex items-center gap-x-6 child-hover:text-primry child:font-IRANYekanBold minidesktop:gap-x-7 child:cursor-pointer">
                    <li>
                        <router-link to="/post" class="text-text_small">پست ها</router-link>
                    </li>
                    <li>
           ...
           <!-- btn switch menu -->
           <div  class="laptop:hidden flex items-center gap-x-5 z-30">
            <div @click="sendValue" class=" bg-select h-[40px] w-[40px] flex items-center justify-center cursor-pointer">
...
<script setup>
 ...
 const sendValue = () => {
   if (show_switch.value) {
    ...
    emit("update-value" , show_switch.value)
 }
</script>